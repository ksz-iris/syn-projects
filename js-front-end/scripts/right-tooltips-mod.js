/**
 * RightJS-UI TooltipMods v2.2.1
 * http://rightjs.org/ui/tooltips
 *
 * Copyright (C) 2009-2011 Nikolay Nemshilov
 */
var TooltipMod = RightJS.TooltipMod = (function(document, RightJS) {
/**
...
var TooltipMod = new Widget({
  extend: {
    ...
    Options: {
      cssRule:    '[data-tooltip-mod]', // a css-marker of an element with a tooltip
      ...
      idSuffix:   '-tooltip-mod'        // ID prefix for tooltips with ID
    },
...
//      if (element.match(TooltipMod.Options.cssRule)) {
      if (element.match(TooltipMod.Options.cssRule)) {
        var uid = $uid(element);
//        return (TooltipMod.instances[uid] || (TooltipMod.instances[uid] = new TooltipMod(element)));
        return TooltipMod.instances[uid];
      }
    }
  },
  ...
  initialize: function(element, generator, options) {
    this.associate = element = $(element);
	 element.set("data-tooltip-mod",true);
    this
      .$super('tooltipMod')
...
//        '</div>'
//      )
		.append($E("div", {class:"rui-tooltip-arrow"}))
		.append($E("div", {class:"rui-tooltip-container"}))
      .on({
  ...
  show: function(immediately) {
    ...
    TooltipMod.instances.each(function(tip) {
    ...
    this._timer = R(function() {
		if (this.generator) {
			var tmp = this.generator();
			this.first(".rui-tooltip-container").insert(Element.initialize("div").text("sds")); 
      ...
      TooltipMod.current = this.fire('show');
    }).bind(this).delay(this.options.delay);

    return (TooltipMod.current = this);
  },
